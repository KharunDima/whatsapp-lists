...
class ResultSaver:
    ...
    async def save_results(self,
        ...
        # 1. Сохраняем домены в TXT
        await self._save_domains_txt(domains)

        # 2. Сохраняем CIDR сети
        await self._save_cidrs(ipv4_cidrs, ipv6_cidrs)

        # 3. Сохраняем полный отчет в JSON
        await self._save_json_report(domains, ipv4_cidrs, ipv6_cidrs, target_config)

    async def _save_domains_txt(self, domains: List[Dict[str, Any]]):
        """Сохраняет домены в TXT файл"""
        filename = f"{self.output_dir}/domains.txt"

        # Сортируем домены
        sorted_domains = sorted(set(domain['domain'] for domain in domains))
        ...
        async with aiofiles.open(filename, 'w', encoding='utf-8') as f:
            ...
            await f.write(f"# Total domains: {len(sorted_domains)}\n")
            await f.write("#" * 50 + "\n\n")
        ...
        logger.info(f"📁 Домены сохранены: {filename} ({len(sorted_domains)} доменов)")
    ...
    async def _save_cidrs(self, ipv4_cidrs: List[str], ipv6_cidrs: List[str]):
        ...
        if ipv6_cidrs:
            ...
            # Фильтруем некорректные CIDR (например, ::/48)
            filtered_ipv6_cidrs = []
        ...
        report = {
            "meta": {
                ...
                "statistics": {
                    ...
                    "unique_domains": len(set(d['domain'] for d in domains)),
                    "domains_with_ips": len([d for d in domains if d.get('ips')]),
            ...
            "domains": domains[:500],  # Ограничиваем для производительности
            "networks": {
                "ipv4": ipv4_cidrs,
                "ipv6": ipv6_cidrs
            },
